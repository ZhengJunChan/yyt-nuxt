...
<template>
  <div class="dynamic_item_component">
    ...
    :class="{
        ...
        <li v-for="(img, index) in info.imglist_info" :style="{backgroundImage: getImg(img)}" v-if="index<6" :key="index">
        </li>                  
    </ul>  
    ...
    <div class="time_line">
      ...
      <span class="fr discuss" v-text="info.comment_text" @click="getDownloadPage()"></span>
      <span class="fr zan" v-text="info.agrees_text" @click="getDownloadPage()"></span>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    getImg(img) {
      ...
      if (this.info.imglist_info.length === 1) {
        return `url(${CommonUtil.fixImg(img.link + '/750')})`
      }

      if (this.info.imglist_info.length % 2 === 0 && this.info.imglist_info.length % 3 !== 0) {
        return `url(${CommonUtil.fixImg(img.link + '/375')})`
      }

      return `url(${CommonUtil.fixImg(img.link + '/230')})`
    }
  }
}
</script>
