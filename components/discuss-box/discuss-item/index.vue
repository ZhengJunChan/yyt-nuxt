...
<template>
  <div class="discuss_item_component">
    <header-img class="user_header" :size="70" :header-img="info.head_info.link" :header-id="info.uid" :vip="info.member_type==2"></header-img>
    ...
    <div class="body">
      ...
      <div class="scroller_label clear_float" :style="{width: scrollerWidth}">
          <div class="img_item fl" v-for="(img, index) in info.imglist_info" :key="index" :style="{backgroundImage: `url(${fixImg(img.link, 'w=110&h=110')})`}"></div>
      </div>
      ...
      <div class="discuss_detail" v-if="info.com_count">
          <p v-for="item in info.com_lists" :key="item.id"
